...
# Reset data frames
ifh.warn("Resetting go.geneid.df and kegg.geneid.df. You have to run next chunks for initialization.")
go.geneid.df <- NA
kegg.geneid.df <- NA
```
...
ifh.table.export(kegg.geneid.info.table, "./gene-to-pref-name.csv", row.names = FALSE, quote = F, sep = ";", col.names = TRUE)
...
ifh.warn(paste(length(kegg.geneid.df$NEW_GENEID[is.na(kegg.geneid.df$GENEID) & kegg.geneid.df$KEGG_Pathway == "-"]), "of the new locus tags have no old locus tag but are associated with a pathway.", sep = " "))
```
...
unmappable.tags <- locus.tag.map$new[is.na(locus.tag.map$old)]
deseq2.unmapped <- deseq2.df[deseq2.df$X.2 %in% unmappable.tags,]
deseq2.unmapped <- data.frame(LocusTag = deseq2.unmapped$X.2, log2FoldChange = deseq2.unmapped$log2FoldChange)
```
...
This chunk will perform the KEGG GSE. It's important that we have a special setup, thus we have to adapt multiple things to work. But this chunk also contains the basic process as used in the online example (see else case).

```{r}
ifh.step("Prepare KEGG gene list...")
ifh.quiet(config$quiet)

# Run KEGG GSE with our special setup
if (length(kegg.geneid.df) > 1) {
    ...
    # Perform basic vanilla KEGG GSE
